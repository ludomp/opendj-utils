...
# Portions Copyright 2012 SmartWave SA.
...
$err[118] = "Canceled\n";
$err[119] = "No Such Operation\n";
$err[120] = "Too Late\n";
$err[121] = "Cannot Cancel\n";
$err[122] = "Assertion Failed\n";
$err[123] = "Authorization Denied\n";
...
print "SORT VLV Operations:          $sortvlv\n";
...
if ($usage =~ /a/i || $verb eq "yes"){
	...
	if ($base_count > 0){
		...
		foreach $bas (sort { $base{$b} <=> $base{$a} } keys %base) {
			if ($bas !~ m/.*cn=config$/){ # Requested by AMR - SMW
...
if ($usage =~ /x/i || $verb eq "yes"){
	...
	if ($extendedop > 0){
		...
		foreach $oids (sort { $oid{$b} <=> $oid{$a} } (keys %oid) ){
			# This is the list of supported Extended Operations in OpenDJ
			if ($oids eq "1.3.6.1.1.8"){ $oidmessage = "Cancel Operation"}
			elsif ($oids eq "1.3.6.1.4.1.26027.1.6.2"){ $oidmessage = "Get Connection ID"}
			elsif ($oids eq "1.3.6.1.4.1.4203.1.11.1"){ $oidmessage = "Password Modify"}
			elsif ($oids eq "1.3.6.1.4.1.26027.1.6.1"){ $oidmessage = "Password Policy State"}
			elsif ($oids eq "1.3.6.1.4.1.1466.20037"){ $oidmessage = "Start TLS"}
			elsif ($oids eq "1.3.6.1.4.1.4203.1.11.3"){ $oidmessage = "Who Am I ?"}
...
sub displayUsage {
	...
	print "- Commandline Switches:\n\n";
        ...
        print "         -N Do NOT display results with config attributes and config suffixes\n";
        ...
        print "                 y       Connection Duration Stats\n";
...
#	if (m/ SORT /){$sortvlv++}
	...
	if (m/ CONNECT conn=0 from/){$restarts++}
	...
	if (m/ EXTENDED REQ .* name=\"StartTLS\"/){$startTLS++;}	# Requested by AMR - SMW
...
#			if ($vlvconn[$i] eq $con && $vlvop[$i] eq $op){ $vlvnotes++; $v="1";}
			if ($vlvconn[$i] eq $con && $vlvop[$i] eq $op){ $vlvnotes++; $v="0";}
		}
		$notes++;
	...
	# grep -e ' RES .* nentries' access_example | cut -d ' ' -f 3 | sort | uniq
	# Result: SEARCH
	...
	#	if (m/ scope=2 /){ $objectclass++;}
		if (m/ scope=wholeSubtree /){ $objectclass++;}
	}
